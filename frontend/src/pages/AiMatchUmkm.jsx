...
export default function AiMatchUmkm() {
  ...
  // popup state — UMKM mendaftar ke event
  const [showModal, setShowModal] = useState(false);
  const [modalMessage, setModalMessage] = useState("");
  const [modalPrice, setModalPrice] = useState("");

  const displayName = user?.name || "Kreasi Lokal";
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] font-sans">
      ...
      <div className="relative overflow-hidden bg-[#0B2340]">
        ...
        <div className="pointer-events-none absolute left-0 bottom-0 hidden text-white/30 lg:block" aria-hidden>
          <img className="h-25 w-25" src="/img/kirim.png" alt="" />
        </div>
        <div className="pointer-events-none absolute right-0 top-0 hidden text-white/25 lg:block" aria-hidden>
        i  <img className="h-23 w-23" src="/img/bintang.png" alt="" />
        </div>
        <div className="relative mx-auto max-w-[1220px] px-5 py-8 md:px-8 md:py-9">
          <h1 className="text-[22px] font-extrabold leading-tight text-white md:text-[28px]">
            Rekomendasi Event untuk <span className="text-white">{displayName}</span>
          </h1>
          <p className="mt-2 max-w-[760px] text-[13px] leading-6 text-white/70">
            Berikut event yang paling cocok dengan profil usaha dan preferensimu. AI kami menganalisis produk, target, lokasi, dan budget untuk hasil terbaik.
          </p>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-[1220px] px-5 py-4 md:px-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="relative flex-1 md:max-w-[520px]">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari event, sekolah, atau UMKM..." className="h-10 w-full rounded-full border border-[#E2E8F0] bg-white pl-10 pr-4 text-sm text-[#0B294D] placeholder:text-[#94A3B8] outline-none focus:border-[#1677FF]" />
            </div>
            <p className="hidden shrink-0 items-center gap-1.5 text-xs text-[#94A3B8] md:flex">
              <span className="h-1.5 w-1.5 rounded-full bg-[#94A3B8]" /> AI Match &gt; UMKM -&gt; Event
            </p>
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              <select value={filterLokasi} onChange={(e) => setFilterLokasi(e.target.value)} className="appearance-none rounded-full border border-[#E2E8F0] bg-white px-4 py-1.5 pr-7 text-xs font-medium text-[#64748B] outline-none">
            ...
            <div className="relative">
              <select value={filterHarga} onChange={(e) => setFilterHarga(e.target.value)} className="appearance-none rounded-full border border-[#E2E8F0] bg-white px-4 py-1.5 pr-7 text-xs font-medium text-[#64748B] outline-none">
            ...
            <span className="ml-auto hidden text-xs text-[#94A3B8] md:inline">{filtered.length} event ditemukan</span>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1220px] px-5 py-6 md:px-8 md:py-6">
        {isLoading ? (
          <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
            <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-[118px] animate-pulse rounded-2xl bg-white shadow-sm" />)}</div>
          ...
          <div className="rounded-2xl border border-[#E2E8F0] bg-white p-12 text-center">
            ...
            <p className="mt-1 text-sm text-[#94A3B8]">Coba ubah filter atau cek lagi nanti. Event published akan muncul di sini.</p>
          </div>
        ) : (
          <div className="grid gap-5 lg:grid-cols-[380px_1fr] lg:items-start">
                  ...
                  <button key={m.event_id} type="button" onClick={() => setSelectedId(m.event_id)} className={`group flex w-full gap-3.5 overflow-hidden rounded-2xl border bg-white p-3 text-left shadow-sm transition ${active ? "border-[#1677FF] ring-1 ring-[#1677FF]/20" : "border-[#E6ECF3] hover:border-[#C9D9EF]"}`}>
                    ...
                    <div className="min-w-0 flex-1 py-0.5">
                      ...
                      <p className="mt-2 flex flex-wrap items-center gap-2 text-[11px] leading-none text-[#94A3B8]">
                        <span className="inline-flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg> {formatDateRange(m.event_date)}</span>
                        <span>📍 {m.location}</span>
                        <span>🏪 {m.booth_capacity} Tenant</span>
                      </p>
            ...
            {selected && (
              <div className="overflow-hidden rounded-2xl border border-[#E6ECF3] bg-white shadow-sm">
                <div className="relative h-[110px] overflow-hidden bg-[#0B2340]">
                  <img src={getEventImage(selected)} alt="" className="h-full w-full object-cover opacity-40" />
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/40 to-transparent p-4 pt-8">
                    <h2 className="text-[18px] font-extrabold text-white">{selected.name}</h2>
                    <p className="mt-1 flex flex-wrap gap-2 text-[11px] text-white/80">
                      <span>{selected.school_name}</span>
                      <span>📅 {formatDateRange(selected.event_date)}</span>
                  ...
                  <div className="absolute right-4 top-1/2 hidden -translate-y-1/2 flex-col items-center md:flex">
                    ...
                    <span className="mt-1 text-[10px] font-semibold text-white/90">Cocok untukmu</span>
                  </div>
                </div>
                <div className="flex items-center justify-between border-b border-[#EEF2F7] bg-[#F8FAFD] px-4 py-3 md:hidden">
                ...
                <div className="border-b border-[#EEF2F7] px-5 py-4 md:px-6">
                  <div className="flex items-center gap-2">
                    {[
                      { n: 1, label: "Pilih Booth", active: true },
                      { n: 2, label: "Kebutuhan Booth", active: false },
                ...
                <div className="px-5 py-5 md:px-6">
                  ...
                  <h3 className="mt-6 text-sm font-bold text-[#0B294D]">Kebutuhan Booth <span className="font-normal text-[#94A3B8]">(Pilih jika diperlukan)</span></h3>
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    <label className="block">
                      ...
                      <input value={displayName} readOnly className="mt-1 h-9 w-full rounded-lg border border-[#E2E8F0] bg-[#F8FAFD] px-3 text-sm text-[#0B294D]" />
                    </label>
                    <label className="block">
                  ...
                  <div className="mt-6 flex flex-col gap-3 border-t border-[#EEF2F7] pt-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-col gap-2">
                      {applyMsg && <p className={`text-xs ${applyMsg.type === "success" ? "text-emerald-600" : "text-red-600"}`}>{applyMsg.text}</p>}
      ...
      {/* POPUP — UMKM daftar ke event (pesan + harga penawaran) */}
      {showModal && selected && (
          ...
          <div className="relative w-full max-w-[520px] overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="border-b border-[#EEF2F7] px-6 py-4">
              ...
              <p className="mt-1 text-xs leading-5 text-[#64748B]">
                Kamu akan mendaftar sebagai tenant di <span className="font-semibold text-[#0B2340]">{selected.name}</span> · {selected.school_name} · Booth {fmtRp(selected.booth_price)}. Tulis pesan untuk sekolah dan harga penawaran jika ingin nego.
              </p>
            </div>
            <div className="space-y-4 px-6 py-5">
              ...
              <label className="block">
                <span className="text-xs font-semibold text-[#0B294D]">Harga Penawaran <span className="font-normal text-[#94A3B8]">(opsional, Rp — kosongkan jika ikut harga booth)</span></span>
